...
@dp.throttled(func.antiflood, rate=20) #120 - секунды после каждого сообщения пользователя, анти-флуд система.
async def h(message: types.Message, state: FSMContext):
	...
	if result[0] == 0:
		if message.chat.id == admin:
			...
			await message.answer('Сообщение успешно отправленно! Ждите Ответа от Администрации')
